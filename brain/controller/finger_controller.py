...
class FingerController:
    ...
    def __init__(self, pi_ip: str = None, pi_port: int = 5001, simulation_mode: bool = False):
        ...
        # Showcase mode: Enhanced logging
        self._showcase_mode = os.getenv("SHOWCASE_MODE", "true").lower() == "true"
        if self._showcase_mode:
            log_info("🎭 SHOWCASE MODE: Enhanced logging and error recovery enabled")
        ...
        if self.api_key:
            ...
            log_warning(
                "⚠️ No API key configured - connection may fail if Pi requires authentication")
    ...
    async def initialize(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                log_info(
                    f"Connecting to finger controller at {self.base_url} (attempt {attempt + 1}/{self.max_retries})...")
                        ...
                        if response.status == 200:
                            ...
                            async with session.get(f"{self.base_url}/status", headers=headers) as status_response:
                                if status_response.status == 200:
                                    ...
                                    emergency = status_data.get(
                                        "emergency_stop", False)
                                    if emergency:
                                        log_warning(
                                            "Hardware in emergency stop mode - attempting reset...")
                                    ...
                                    raise Exception(
                                        "Authentication failed - check MAXI_HAND_API_KEY")
                                else:
                                    raise Exception(
    ...
    async def set_initial_closed_state(self):
        ...
        try:
            ...
            self.current_pose = {
                "left": [0, 0, 0, 0, 0], "right": [0, 0, 0, 0, 0]}
            self.target_pose = {
                "left": [0, 0, 0, 0, 0], "right": [0, 0, 0, 0, 0]}
            self.fingers_moving = {"left": [False] * 5, "right": [False] * 5}
            ...
            try:
                ...
                if attempt < retries - 1:
                    delay = self.retry_delays[min(
                        attempt, len(self.retry_delays) - 1)]
                    log_warning(
    ...
    async def set_finger_pose(self, hand: str, pose: List[int], duration_ms: int = 300) -> bool:
        ...
        try:
            ...
            for i, value in enumerate(pose):
                if value not in [0, 1]:
                    raise ValueError(
                        f"Pose values must be 0 or 1, got {value} at index {i}")
            ...
            individual_duration = max(
                120, duration_ms // max(1, len(movements_needed)))
    ...
    async def show_number(self, number: int, hand_preference: str = "right") -> bool:
        ...
        try:
            ...
            log_info(
                f"Showing number {number} with {hand_preference} hand preference")
            ...
            if number == 0:
                ...
                partial_hand_pose = [
                    1 if i < remainder else 0 for i in range(5)]

                other_hand = "left" if hand_preference == "right" else "right"

                # OPTIMIZED: Execute both hands with minimal delay
                task1 = self.set_finger_pose(
                    hand_preference, full_hand_pose, 250)
                await asyncio.sleep(0.05)  # OPTIMIZED: Reduced delay
                task2 = self.set_finger_pose(
                    other_hand, partial_hand_pose, 250)
