...
def extract_math_problem(text: str):
    ...
    multi_step_match = re.search(
        r'add\s+([a-z\s\-\d]+)\s+and\s+([a-z\s\-\d]+)\s+then\s+divide\s+by\s+([a-z\s\-\d]+)', text)
    if multi_step_match:
    ...
    word_expr = re.search(
        r'(what\s+is\s+)?([a-z\s\-\d]+)\s+(plus|minus|times|divided by|multiplied by|over|add|subtract|multiply|divide|greater than|less than|equals|equal to|is)\s+([a-z\s\-\d]+)', text)
    if word_expr:
