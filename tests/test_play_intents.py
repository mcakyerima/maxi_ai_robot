...
print("\nspelling (SPEAK 'B! A! N...' hack, SHOW clean letters):")
spoken, display = spell_word_reply("cat")
check("spoken uses caps + '!' (last has none)", "C! A! T" in spoken and "T!" not in spoken)
check("display shows clean letters", display == "Cat is spelled C - A - T. Cat!")
check("spoken != display", spoken != display)
sp_b, di_b = spell_word_reply("banana")
check("banana spoken = B! A! N! A! N! A", "B! A! N! A! N! A" in sp_b)
check("banana display letters", "B - A - N - A - N - A" in di_b)
g_spoken, g_display = spell_game_reply(0)
check("game spoken has caps + '!' + invite", "!" in g_spoken and "try" in g_spoken.lower())
check("game display has clean letters + invite", " - " in g_display and "try" in g_display.lower())
